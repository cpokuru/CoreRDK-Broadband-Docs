...
JS = textwrap.dedent(
    ...
    function renderDonutChart() {
      ...
      function draw(progress) {
        ...
        // Segment labels for big segments (>8%) — only after animation completes
        if (p > 0.95) {
          segments.forEach((s) => {
            const sweep = s.end - s.start;
            const pctSeg = sweep / (2 * Math.PI);
            if (pctSeg < 0.08) return;
            const mid = (s.start + s.end) / 2;
            const labelR = R + 18;
            const lx = cx + Math.cos(mid) * labelR;
            const ly = cy + Math.sin(mid) * labelR;
            const endX = cx + Math.cos(mid) * (R + 8);
            const endY = cy + Math.sin(mid) * (R + 8);
            ctx.beginPath();
            ...
            ctx.fillText(short, lx + (lx > cx ? 3 : -3), ly + 3);
          });
        }
        ...
        ctx.textAlign = 'left';
        const itemH = Math.min(22, (H - 10) / entries.length);
        entries.forEach(([label, count], i) => {
          const ly = 8 + i * itemH;
          if (ly + itemH > H) return;
          const isHov = anim.hoveredIdx === i;
          // Color square
          ctx.fillStyle = PALETTE[i % PALETTE.length];
          ctx.fillRect(lx0, ly + 2, 10, 10);
          // Label
          ctx.fillStyle = isHov ? PALETTE[i % PALETTE.length] : textSec;
          ctx.font = isHov ? 'bold 9.5px Inter, sans-serif' : '9.5px Inter, sans-serif';
          const short = label.length > 16 ? label.slice(0, 15) + '…' : label;
          ctx.fillText(short, lx0 + 14, ly + 11);
          // Count
          ctx.fillStyle = textMuted;
          ctx.font = '8.5px Inter, sans-serif';
          ctx.fillText(count, lx0 + 14, ly + itemH - 2);
          // Mini progress bar
          const barW = legendW - 28;
          ctx.fillStyle = themeVar('--border', '#e5e7eb');
          ctx.fillRect(lx0 + 14, ly + itemH - 1, barW, 2);
          ctx.fillStyle = PALETTE[i % PALETTE.length];
          ctx.fillRect(lx0 + 14, ly + itemH - 1, Math.round((count / total) * barW), 2);
        });
      }
                ...
                <div id="main-content" style="display:none">
                  ...
                  <div id="charts-row">
                    <div class="chart-card">
                      ...
                      <canvas id="donut-canvas" width="500" height="320" style="width:100%;height:auto;max-height:320px"></canvas>
